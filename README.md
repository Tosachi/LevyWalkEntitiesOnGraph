...
このシミュレータでは信貴氏、西田氏が提案しユニットディスクグラフ上での Levy walk のアルゴリズムを元に実装している。ユニットディスクグラフ上での Levy walk では、1ステップで移動するノード数の分布が式(1)の冪分布に従う。  
  ...
  <img src="https://latex.codecogs.com/svg.image?P(l)&space;\propto&space;l^{-\frac{1}{\lambda}}&space;(0.0&space;<&space;l&space;<&space;1.0,&space;1.0&space;<&space;\lambda&space;\le&space;3.0)" title="P(l) \propto l^{-\frac{1}{\lambda}} (0.0 < l < 1.0, 1.0 < \lambda \le 3.0)" />  ・・・(1)  
